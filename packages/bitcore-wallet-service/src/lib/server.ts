...
export class WalletService {
  ...
  /**
   * Fetch recent notification by copayerId from storage.
   * @param {Object} opts
   * @returns {Object} notification
   */
   fetchNotificationsByWalletId(opts, cb) {
    opts = opts || {};
    this.storage.fetchNotificationsByWalletId(opts.walletId, opts.type, opts.limitNumber, (err, notifications) => {
      if (err) return cb(err);
      if (!notifications) return cb(Errors.NOTIFICATION_NOT_FOUND);
